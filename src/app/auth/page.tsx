    ...
    <div aria-hidden="true" className="pointer-events-none absolute inset-0 overflow-hidden">
      ...
      <svg className="absolute inset-0 h-full w-full opacity-[.13]" viewBox="0 0 100 100" preserveAspectRatio="none">
        <defs>
          <pattern id="auth-grid" width="8" height="8" patternUnits="userSpaceOnUse">
            <path d="M 8 0 L 0 0 0 8" fill="none" stroke="white" strokeOpacity=".11" strokeWidth=".12" />
          </pattern>
          <linearGradient id="auth-line" x1="0" y1="1" x2="1" y2="0">
            <stop offset="0" stopColor="#22d3ee" stopOpacity="0" />
            <stop offset=".35" stopColor="#22d3ee" stopOpacity=".45" />
      ...
      <svg className="absolute -right-8 top-[13%] h-[360px] w-[480px] opacity-[.10]" viewBox="0 0 100 50" preserveAspectRatio="none">
        {candles.map(([x, high, low, open, close, bodyHigh], index) => {
          ...
          return (
            <g key={index}>
              <line x1={x} y1={low} x2={x} y2={high} stroke="#67e8f9" strokeWidth=".45" />
              <rect x={x - 1.8} y={bodyY} width="3.6" height={bodyH} rx=".4" fill={rising ? "#22d3ee" : "#94a3b8"} fillOpacity=".72" />
              <line x1={x - 1.8} y1={bodyHigh} x2={x + 1.8} y2={bodyHigh} stroke="white" strokeOpacity=".25" strokeWidth=".25" />
            </g>
          );
        })}
      ...
      <div className="relative z-10 mx-auto flex min-h-screen w-full max-w-5xl items-center justify-center px-5 py-10">
        <section className="w-full max-w-[400px]">
          <div className="mb-10 text-center">
            <div className="mx-auto mb-5 flex h-11 w-11 items-center justify-center rounded-xl border border-cyan-300/10 bg-white/[.035] shadow-[0_0_30px_rgba(34,211,238,.08)]">
