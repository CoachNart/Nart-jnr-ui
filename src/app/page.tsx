        ...
        <div className="p-5">
            ...
            <div className="mb-4 flex items-center justify-between">
              <div>
                ...
                <p className="mt-1 text-[10px] text-zinc-700">
                  Trade levels generated by Nart Jnr
                </p>
              </div>
          ...
          <div className="mt-5 flex items-center justify-between border-t border-white/[0.05] pt-4">
            ...
            <span className="font-mono text-[7px] tracking-[0.16em] text-zinc-700">
              NART JNR / {locked ? "PREMIUM SIGNAL" : "PUBLIC SIGNAL"}
            </span>
  ...
  useEffect(() => {
    ...
    async function loadAnalysis() {
      try {
        ...
        const response = await fetch(
          `${base}/api/analysis?symbol=BTCUSDT`,
          {
            cache: "no-store",
          }
        );
        ...
        if (!payload.ok) {
          throw new Error(
            payload.error || "Nart Jnr API error"
          );
        }
    ...
    async function loadSetups() {
      try {
        ...
        if (!payload.ok) {
          throw new Error(
            payload.error || "Signals API error"
          );
        }
        ...
        if (!cancelled) {
          setLiveSetups(signals as NartSetup[]);
        }
      } catch (error) {
        console.error(
          "❌ Live signal loading failed:",
          error
        );
      }
    }

    async function loadLivePrices(signals: NartSetup[]) {
      const base =
        process.env.NEXT_PUBLIC_NART_API ||
        "http://127.0.0.1:8787";
      ...
      const updated = await Promise.all(
        signals.map(async (signal) => {
          try {
            const response = await fetch(
              `${base}/api/analysis?symbol=${encodeURIComponent(signal.symbol)}`,
              { cache: "no-store" }
            );
      ...
      if (!cancelled) {
        setLiveSetups(updated);
      }
    }

    async function refreshSetupsAndPrices() {
      try {
        const base =
          process.env.NEXT_PUBLIC_NART_API ||
          "http://127.0.0.1:8787";
        ...
        await loadLivePrices(signals as NartSetup[]);
      } catch (error) {
        console.error(
          "❌ Live signal/price loading failed:",
          error
        );
      }
    }
    ...
    loadSetups();
    refreshSetupsAndPrices();

    const signalRefresh = setInterval(() => {
      if (!cancelled) {
        refreshSetupsAndPrices();
      }
    }, 5 * 60 * 1000);
          ...
          <div className="mb-10">
            ...
            <p className="mt-3 text-sm leading-6 text-zinc-500">
              {authMode === "login"
                ...
                : "Create your Nart Jnr account and start tracking the market."}
            </p>
          </div>

          <div className="rounded-[28px] border border-white/[0.07] bg-white/[0.025] p-5 shadow-2xl">

            <div className="mb-5 grid grid-cols-2 rounded-2xl bg-black/30 p-1">
              <button
                type="button"
                onClick={() => {
                  setAuthMode("login");
                ...
                className={`rounded-xl px-3 py-2.5 text-[10px] font-bold tracking-[0.12em] transition ${
                  authMode === "login"
            ...
            <input
              ...
              autoComplete="email"
              className="mt-2 mb-4 w-full rounded-2xl border border-white/[0.07] bg-black/30 px-4 py-3.5 text-sm text-white outline-none placeholder:text-zinc-700 focus:border-cyan-400/30"
            />
            ...
            <input
              ...
              className="mt-2 w-full rounded-2xl border border-white/[0.07] bg-black/30 px-4 py-3.5 text-sm text-white outline-none placeholder:text-zinc-700 focus:border-cyan-400/30"
            />
            ...
            <button
              ...
              onClick={submitAuth}
              className="mt-5 w-full rounded-2xl bg-cyan-400 px-4 py-3.5 text-[10px] font-black tracking-[0.16em] text-black transition hover:bg-cyan-300 disabled:cursor-not-allowed disabled:opacity-50"
            >
              {authSubmitting
                ? "PLEASE WAIT..."
                : authMode === "login"
  ...
  const displaySetups = liveSetups.map((item) => {
        ...
        return {
          ...
          thesis:
            plan.reason?.length
              ? plan.reason.join(" • ")
              : "Nart Jnr is monitoring this market.",
        };
      });
              ...
              {displaySetups.length > 0 ? (
                ...
                <div className="px-5 py-10 text-center">
                  ...
                  <p className="mx-auto mt-1 max-w-xs text-[10px] leading-5 text-zinc-600">
                    Nart Jnr is watching the market. A setup appears here only after confirmation.
                  </p>
          ...
          <section className="py-8">
            ...
            <div className="mb-6">
              ...
              <p className="mt-2 max-w-xl text-sm leading-6 text-zinc-500">
                Live market intelligence generated by the Nart Jnr engine.
              </p>
            </div>
            ...
            {liveAnalysis?.tradePlan ? (
              ...
              <div className="rounded-3xl border border-white/[0.06] bg-white/[0.02] p-8 text-center">
                <p className="text-sm text-zinc-500">
                  {apiLoading
                    ? "Synchronizing with Nart Jnr engine..."
            ...
            {selectedSetup.premium ? (
              <div className="nart-premium-glow overflow-hidden rounded-3xl border border-amber-400/15 bg-gradient-to-br from-amber-400/[0.07] via-cyan-400/[0.025] to-transparent p-6">
                ...
                <p className="mt-3 max-w-xl text-sm leading-6 text-zinc-500">
                  ...
                  complete Nart Jnr analysis, execution plan and monthly setup
                  allocation.
                </p>
                ...
                <div className="mt-4 grid gap-4 sm:grid-cols-2">
                  ...
                  <div className="nart-card rounded-3xl border border-white/[0.06] p-5">
                    ...
                    <p className="mt-3 text-xs leading-6 text-zinc-500">
                      Nart Jnr identified directional structure aligned with
